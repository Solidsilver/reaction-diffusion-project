...
__constant__ float weights[3][3] = {{0.05, 0.2, 0.05},
                       {0.2, -1, 0.2},
                       {0.05, 0.2, 0.05}};
...
void init_map(float *** arr)
{
    for (int i = 0; i < HEIGHT; i++)
    {
        cudaMallocManaged(&(arr[i]), WIDTH * sizeof(float*));
        // arr[i] = new float *[WIDTH];
        for (int j = 0; j < WIDTH; j++)
        {
            cudaMallocManaged(&(arr[i][j]), 2 * sizeof(float));
            // arr[i][j] = new float[2];
            arr[i][j][A] = 1;
            if (rand() % 100 < RANDOM)
            {
...
int main()
{
    ...
    float ***cur;
    float ***prev;
    cudaMallocManaged(&cur, HEIGHT * sizeof(float**));
    cudaMallocManaged(&prev, HEIGHT * sizeof(float**));
    init_map(cur);
    init_map(prev);
